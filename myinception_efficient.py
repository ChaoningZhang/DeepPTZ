...
class MyInception3_siamese_efficient(nn.Module):
    def __init__(self, siamese=True, pretrained = True, pretrained_fixed=True,aux_logits=False):
        super(MyInception3_siamese_efficient, self).__init__()
        input_channels = 3
        num_classes = 3
        num_classes_extra = 4
        ...
        if not siamese:
            input_channels = 6
            pretrained = False # pretrained parameter cannot be used when non-siamese
    ...
    def forward(self, input1, input2):
        ...
        if self.siamese:
            ...
            #norm1 = torch.mean(torch.mean(torch.norm(feature1, 2,1,True),3,True),2,True)
            #norm2 = torch.mean(torch.mean(torch.norm(feature2, 2,1,True),3,True),2,True)
            #norm1 = torch.mean(torch.norm(feature1, 2,1))
            #norm2 = torch.mean(torch.norm(feature2, 2,1))
            #pdb.set_trace()
            #feature1 = norm1*F.normalize(feature1, p=2,dim=1,eps=1e-12)
            #feature2 = norm2*F.normalize(feature2, p=2,dim=1,eps=1e-12)
            #out_conv_redir = self.conv_redir(feature1)
            #out_correlation = correlate(feature1, feature2)
            out_correlation1 = self.FeatureCorrelation(feature1, feature2)
            out_correlation2 = self.FeatureCorrelation(feature2, feature1)
            #out_correlation = out_correlation*(1+out_correlation)
            #out_correlation = self.SELayer(out_correlation)
            #pdb.set_trace()
            #feature = self.conv_all(torch.cat([out_conv_redir, out_correlation], dim=1))
            feature1 = self.conv_all(torch.cat([out_correlation1], dim=1))
            feature2 = self.conv_all(torch.cat([out_correlation2], dim=1))
        else:
            feature = self.forward_once(torch.cat((input1, input2), 1))
        #feature  = torch.cat((feature1, feature2), 1) #8 x 8 x 2048*2
        x1        = self.relation(feature1) # .detach()
        x2        = self.relation(feature2) # .detach()
        x         = torch.cat((x1, x2), 1)
        return x
